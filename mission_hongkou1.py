...
def protect(slide_box,magic_box,frame,width,height,conn):
    ...
    # =============================================================================
    t0 = time.time()
    time_person = slide_box.groupby(['person_id'])['timestamp'].max()
    delete_persons = []
    for t in time_person:
        if time.time() - t / 1000 > 15 * 4:
            #                  print(time_person[time_person.values == t].index[0])
            delete_persons.append(time_person[time_person.values == t].index[0])
    delete_persons = set(delete_persons)
    for i in delete_persons:
        slide_box = slide_box[~slide_box['person_id'].isin([i])]
    # print('delete watestd time %5f'%(time.time()-t0))
    # =============================================================================
    #     需要判别是否溺水的人
    # =============================================================================
    t0 = time.time()
    ...
    for j in range(len(need_cal_person)):
        ...
        judge_person = judge_person.sort_values(['timestamp'], ascending=False)
        judge_person = judge_person.iloc[:70][:]  # 15
        judge_person = judge_person.reset_index(drop=True)
        # 动、静态溺水判别
        t1 = time.time()
        recognition = discriminantStatic(judge_person,width,height)
        if recognition == 0:  # 为0 alarm
...
def hardwork(name,det,tracker,url,conn,line1,line2,screenshot):
    ...
    while True:
        ...
        cv2.line(frame, spot3, spot4, 2, lineType=cv2.LINE_AA)
...
def main():
    ...
    # url1 = r'/home/nvidia/Video_data/alarm_test.mp4'
    url1 = r'rtsp://admin:xike123456@fanmaoyang.kmdns.net:58084/stream1'
    qmap['1'] = conn1
    line1 = ((0,0),(0,0))
    line2 = ((0,0),(0,0))
    # screenshot = [402,895, 97,1920]
    screenshot  = [606,1260,126,2413] #2560*1440
    go1 = threading.Thread(target=hardwork, name='stream1', args=('work1',det,tracker1,url1,conn1,line1,line2,screenshot))
    go1.start()

    line1 = ((0, 0), (0, 0))
    line2 = ((0, 0), (0, 0))
    screenshot  = [606,1260,126,2413]
